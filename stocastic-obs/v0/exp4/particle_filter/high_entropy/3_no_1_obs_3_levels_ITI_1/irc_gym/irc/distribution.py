...
from .alias import Array, Tensor, Module, Optimizer, VarSpace, RandGen
from .utils import progress_summary

defaults_dir = Path(__file__).parent/'defaults'
with open(defaults_dir/'distribution.yaml') as f:
    DIST_CONFIG = Config(yaml.safe_load(f))
...
class BasicDiscretePotential(BasePotential):
    ...
    def __init__(self,
        ...
        """
        super(BasicDiscretePotential, self).__init__()
        self.nvec = nvec
...
class BaseParamNet(Module):
    ...
    def forward(self,
        ...
        Returns
        -------
        param_vecs: (num_samples, num_params)
...
class CompleteEmbedParamNet(BaseParamNet):
    ...
    MultiDiscrete variables are treated as a one-hot vector, and an MLP is added
    further optionally.
...
class BaseDistribution(Module):
    ...
    def __init__(self,
        ...
        phis: Optional[list[Union[BasePotential, Config, None]]] = None,
        y_space: Optional[VarSpace] = None,
        param_net: Union[BaseParamNet, Config, None] = None,
        ...
        param_net:
            The parameter network for conditional distribution or the
            configuration to instantiate one.
        rng:
        ...
        for idx, phi in zip(self.idxs, phis):
            if phi is None or isinstance(phi, Config):
                if isinstance(self.x_space, MultiDiscrete):
                    phi_config = Config(phi).fill(DIST_CONFIG.potential.discrete)
                    phi = phi_config.instantiate(nvec=self.x_space.nvec[idx])
                if isinstance(self.x_space, Box):
                    phi_config = Config(phi).fill(DIST_CONFIG.potential.continuous)
                    phi = phi_config.instantiate(num_vars=len(idx))
        ...
        if self.y_space is None:
            ...
            if param_net is None or isinstance(param_net, Config):
                if isinstance(self.y_space, MultiDiscrete):
                    param_net_config = Config(param_net).fill(DIST_CONFIG.param_net.discrete)
                if isinstance(self.y_space, Box):
                    param_net_config = Config(param_net).fill(DIST_CONFIG.param_net.continous)
                param_net = param_net_config.instantiate(
        ...
        self.est_stats = {} # latest stats for estimating distribution parameters
    ...
    def estimate(self,
        ...
        *,
        ...
        optimizer: Union[Optimizer, Config, None] = None,
        verbose: int = 2,
    ):
        r"""Estimates p(x) or p(x|y) from samples.

        If `ys` is not provided, estimate a distribution p(x) for `xs`,
        otherwise estimate a distribution p(x|y) for the pairs. Estimation is
        done by gradient ascent on log likelihood function. The estimation
        process will be summarized and used to update `self.est_stats`.
        ...
        num_epochs:
            The number of epochs for optimization. For easy implementation with
            numpy random generator, batch is sampled with replacement and
            `num_epochs` actually specifies the total number of batches.
        batch_size:
            Batch size for optimization.
        optimizer:
            A pytorch optimizer, usually an SGD optimizer. It can also be an
        ...
        num_epochs = num_epochs or DIST_CONFIG.estimate.num_epochs
        batch_size = batch_size or DIST_CONFIG.estimate.batch_size
        if optimizer is None or isinstance(optimizer, Config):
            optimizer = Config(optimizer).fill(DIST_CONFIG.estimate.optimizer).instantiate(
        ...
        toc = time.time()
        steps = np.arange(1, len(losses)+1) # optimization steps
        losses = np.array(losses) # training loss at each step
        queries, fits, optimality, fvu = progress_summary(steps, losses)
        self.est_stats = {
            'num_epochs': num_epochs, 'batch_size': batch_size, 'num_samples': num_samples,
...
class DiscreteDistribution(BaseDistribution):
    ...
    def __init__(self,
        ...
        **kwargs,
        ...
        self._all_xs = np.arange(np.prod(self.x_space.nvec))
        self._all_xs = np.stack(np.unravel_index(self._all_xs, self.x_space.nvec)).T

    def __repr__(self):
        r_str = 'Discrete distribution over space {}'.format(self.x_space.nvec)
        if self.y_space is not None:
            r_str += ' conditioned on value in space {}'.format(self.y_space.nvec)
        return r_str
