...
# food_reward_list=(0 5 10 15 20 25 50 75 100 200 300 400 500 600 1000)
food_reward_list=(0.01 10 20 30 40 50 75 100 125 150 175 200)
...
for ((inter_num_epochs = $start_num_epochs; inter_num_epochs <= ${num_epochs}; inter_num_epochs+= $step_num_epochs)); do 
    for ((seed_value = 0; seed_value < ${num_seeds}; seed_value++)); do 
        for ((exp_ind = 0; exp_ind < ${#food_reward_list[@]}; exp_ind++)); do 
            python train.py ${food_reward_list[$exp_ind]} $att_coeff $att_temp $penalty_cost $inter_num_epochs $seed_value &
        done
    done
    wait
    food_reward_list_str="${food_reward_list[@]}"
    python compare_across_seeds.py "$food_reward_list_str" $att_coeff $att_temp $penalty_cost $inter_num_epochs $seed_value $no_episodes &
    wait
done
wait
