...
def plot_AF_episode(episode, env, agent, nodes_from_zero = 20, time_steps_before_lick = 10):        
    ...
    for attention_choice in range(no_attention_modes):
        ...
        attention_choice_idxs.append(list(map(lambda action_choice: True if (action_choice[1] == attention_choice)  else False, offset_actions)))
    ...
    # Commented as these visualizations are extra.
    # plt.figure()
...
import os
def find_recent_epoch(directory, pattern):
    highest_value = -1
    ...
    if highest_value_file:
        print(f"Highest epoch saved is {highest_value}")
