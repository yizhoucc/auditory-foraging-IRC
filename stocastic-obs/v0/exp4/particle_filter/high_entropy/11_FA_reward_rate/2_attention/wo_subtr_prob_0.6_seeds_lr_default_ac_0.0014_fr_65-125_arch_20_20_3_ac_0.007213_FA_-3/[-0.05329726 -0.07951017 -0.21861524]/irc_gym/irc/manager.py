    ...
    def setup(self, config):
        ...
        class CustomNetwork(nn.Module):
            ...
            def __init__(
                ...
                #     nn.Linear(feature_dim, 20), nn.Tanh(), nn.Linear(20, 20), nn.Tanh(), nn.Linear(20, last_layer_dim_pi), nn.Tanh()
                # )
                # self.policy_net = nn.Sequential(
                #     nn.Linear(feature_dim, 20), nn.Tanh(), nn.Linear(20, 20), nn.Tanh(), nn.Linear(20, 3), nn.Tanh()
                # )
                
                # removing subtraction
                # self.policy_net = nn.Sequential(
                #     nn.Linear(feature_dim, 20), nn.Tanh(), nn.Linear(20, 20), nn.Tanh(), nn.Linear(20, 3)
                # )
                self.policy_net = nn.Sequential(
                    nn.Linear(feature_dim, 20), nn.Tanh(), nn.Linear(20, 20), nn.Tanh(), nn.Linear(20, 4)
                )

                # Value network
                # self.value_net = nn.Sequential(
                #     nn.Linear(feature_dim, 20), nn.Tanh(), nn.Linear(20, 20), nn.Tanh(), nn.Linear(20, last_layer_dim_vf), nn.Tanh()
                # )
                self.value_net = nn.Sequential(
                    nn.Linear(feature_dim, 20), nn.Tanh(), nn.Linear(20, 20), nn.Tanh(), nn.Linear(20, last_layer_dim_vf)
                )
            ...
            def forward_actor(self, features: th.Tensor) -> th.Tensor:
                ...
                # removing subtraction
                # mid_features =  self.policy_net(features)
                # diff_1_2 = mid_features[:, 1] - mid_features[:, 0]
                # diff_1_3 = mid_features[:, 2] - mid_features[:, 0]
                # final_features = torch.stack((-diff_1_2, diff_1_2, -diff_1_3, diff_1_3), dim=1)
                mid_features =  self.policy_net(features)
                final_features = torch.stack((-mid_features[:, 0], mid_features[:, 0], mid_features[:, 1], mid_features[:, 2], mid_features[:, 3]), dim=1)
        ...
        # Usage example:
        algo = config.algo.instantiate(
            policy=CustomActorCriticPolicy, env=model,
            device=self.device, seed=config.seed, gamma=config.algo.gamma, ent_coef=config.algo.ent_coef, learning_rate=config.algo.learning_rate
        )
...
class IRCManager:
    ...
    def agent_trained_wegihts_biases(self,
        ...
        weights_bias_dict['policy_net_weight'] = self.agent_manager.ckpt['agent_state']['policy_state']['policy_net.weight']
        weights_bias_dict['policy_net_bias'] = self.agent_manager.ckpt['agent_state']['policy_state']['policy_net.bias']
        weights_bias_dict['value_net_weight'] = self.agent_manager.ckpt['agent_state']['policy_state']['value_net.weight']
        weights_bias_dict['value_net_bias'] = self.agent_manager.ckpt['agent_state']['policy_state']['value_net.bias']

        features = self.agent.algo.policy.extract_features(obs)
        policy_net_0_results = self.agent.algo.policy.mlp_extractor.policy_net[0](features)
        policy_net_1_results = self.agent.algo.policy.mlp_extractor.policy_net[1](policy_net_0_results)
        policy_net_2_results = self.agent.algo.policy.mlp_extractor.policy_net[2](policy_net_1_results)
        policy_net_3_results = self.agent.algo.policy.mlp_extractor.policy_net[3](policy_net_2_results)
        policy_net_4_results = self.agent.algo.policy.action_net(policy_net_3_results)
