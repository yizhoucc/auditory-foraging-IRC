...
# # Lokesh added this method for sampling action probabilities for Discrete action space.
...
# Lokesh added this method for sampling action probabilities for MultiDiscrete action space. This needs to be changed depending on action space. More work needed. Temporary fix.
    def agent_action_distribution(self,
            beliefs = None,
        ) -> list:
            r"""Returns action distributions for given belief vectors.
            ...
            for belief in beliefs:
                ...
                for action in [[[0, 0]], [[0, 1]], [[1, 0]], [[1, 1]]]:
                    dict[tuple(action[0])] = np.exp(pi.log_prob(torch.tensor(action, dtype=torch.long, device=device)).item())
                action_distributions.append(dict)
            # self.algo.policy.set_training_mode(_to_restore_train)
            return action_distributions