...
class ParticleFilter():
    ...
    def filter(self, lick_actions, state_list, no_particles = None, sampling_freq = None, do_save = False):
        ...
        PF_IO:
            ...
            particle_filter_output:
                ...
                'particles_log_likelihoods': 
                    A numpy array containing the log likelihood value of each particle's trajectory.
        ...
        particle_filter_output = {}
        generated_episodes = []
        sorted_indices = np.argsort(-1 * particles_log_likelihoods)
    ...
    def compute_posterior(self, PF_IO):
        ...
        for action_seq, count in element_counts.items():
            ...
            action_seq_ranked.append(copy.deepcopy(np.array(action_seq)))
            attention_seq_ranked.append(copy.deepcopy(np.array(action_seq)%self.env.no_attention_modes))
            IRC_log_likelihood_ranked.append(copy.deepcopy(PF_IO['output']['particles_log_likelihoods'][generated_actions.index(list(action_seq))]))
