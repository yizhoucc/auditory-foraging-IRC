...
if len(sys.argv) < 9:
    print("ERROR! Correct usage is python compare_across_seeds.py <food_reward_list> <att_coeff_list> <att_temp_list> <penalty_cost_list> <time_in_game_reward> <num_epochs> <seed_list> <no_episodes>")
    sys.exit(1)
else:
    ...
    time_in_game_reward_list = [float(elt) for elt in sys.argv[5].split()] 
    num_epochs = int(sys.argv[6].split()[0])
    seed_list = [elt for elt in range(int(sys.argv[7].split()[0]))]
    no_episodes = int(sys.argv[8])
    lick_cost_list = [0]
    iti_cost_list = [0]
            ...
            for attention_cost_temp in att_temp_list:
                for penalty_cost in penalty_cost_list:
                    for iti_cost in iti_cost_list:
                        for time_in_game_reward in time_in_game_reward_list:
                            ...
                            hit_reaction_time_list.append([])
                            fa_reaction_time_list.append([])
                            env_param = [lick_cost, food_reward, attention_cost_coeff, attention_cost_temp, penalty_cost, iti_cost, time_in_game_reward]
                            print("************************************************************************")
                            ...
                            for seed in seed_list:
                                ...
                                for episide_no in range(no_episodes):
                                    ...
                                    # if episode['actions'][-1] >= env.no_attention_modes:
                                    ...
                                    attention_time_points_across_episodes.append(np.where([elt[1] for elt in episode['actions']] == 1)[0])
                                ...
                                hit_reaction_time_list[-1].append(hit_reaction_time_across_episodes)
                                fa_reaction_time_list[-1].append(fa_reaction_time_across_episodes)
