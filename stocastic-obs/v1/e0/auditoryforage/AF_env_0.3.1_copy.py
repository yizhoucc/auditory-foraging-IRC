...
with open(Path(__file__).parent/'defaults.yaml') as f:
    D_ENV_SPEC = Config(yaml.safe_load(f)).auditory_foraging
...
def fill_dictionary_depth_2(input_dict, default_dict):
            ...
            for key1 in default_dict.keys():
                ...
                for key2 in default_dict[key1].keys():
                    try:
                        filled_dictionary[key1][key2] = input_dict[key1][key2]
                    except:
                        filled_dictionary[key1][key2] = default_dict[key1][key2]
