...
class FoodBoxesEnv(Env):
    ...
    def __init__(self,
        ...
        self.param_low = np.array([0.]*(3*self.num_boxes+2)+[-np.inf])
        self.param_high = np.array([1.]*(3*self.num_boxes+1)+[np.inf, 0])

        # box states and position
...
class IdenticalBoxesEnv(FoodBoxesEnv):
    ...
    def __init__(self,
        ...
        **kwargs,
        ...
        self.param_low = np.array([0, 0, 0, 0, 0, -np.inf])
        self.param_high = np.array([1, 1, 1, 1, np.inf, 0])
...
class SingleBoxEnv(FoodBoxesEnv):
    r"""Foraging environment with one color-cued box.

    Only one box exists in the environment, and the agent is assuemd to always
    stay at the box. The state space and observation space will be simplified
    because agent position is no longer relevant.

    """

    def __init__(self,
        num_shades: Optional[int] = None,
        p_appear: Optional[float] = None,
        p_vanish: Optional[float] = None,
        p_cue: Optional[float] = None,
        r_food: Optional[float] = None,
        seed: Optional[int] = None,
    ):
        super().__init__(
            1, num_shades, p_appear, p_vanish, p_cue, 0, r_food, 0, seed,
        )

        # box state
        self.state_space = MultiDiscrete([2])
        # wait or fetch
        self.action_space = Discrete(2)
        # color cue
        self.observation_space = MultiDiscrete([self.num_shades])

        self.locs = np.zeros((1, 2))

    def get_param(self):
        env_param = np.array([
            *self.p_appear, *self.p_vanish, *self.p_cue, self.r_food,
        ])
        return env_param

    def set_param(self, env_param):
        assert len(env_param)==4
        self.p_appear = np.array(env_param[:1])
        self.p_vanish = np.array(env_param[1:2])
        self.p_cue = np.array(env_param[2:3])
        self.r_food = env_param[3]
    ...
    def set_state(self, state):
        assert len(state)==1
        super().set_state(list(state)+[0])

    def transition_step(self, action):
        if action==0:
            reward, terminated = super().transition_step(0)
        if action==1:
            reward, terminated = super().transition_step(2)
        return reward, terminated

    def observe_step(self):
        self.pos = 0
        observation = super().observe_step()[:1]
        return observation
