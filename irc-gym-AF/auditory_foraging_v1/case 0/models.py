# Lokesh added this numpy import
import numpy as np
...
class BeliefModel(gym.Env):
    ...
    def update_belief(self, action, obs):
        ...
        for _ in range(self.est_spec['belief']['num_samples']):
            ...
            #Lokesh added temporarily for debugging
            ...
            # Lokesh changed this part to include explicit p(o|s,a)
            # weights.append(np.exp(self.p_o_s.loglikelihood(
            #     np.array(obs)[None], np.array(next_state)[None]).item()))
            weights.append(self.af_cond_obs_prob(
                np.array(obs)[None][0][0], np.array(next_state)[None][0][0],action))            
    ...
    # Lokesh added this method to include explicit p(o|s,a). Highly inefficient and does not check for mistakes.
    # Lokesh cond dist for auditory_foraging_v0
    def af_cond_obs_prob(self,
        obs,
        state,
        action,
        no_signal_nodes = 4,
        attention_based_obs = np.array([0,0.5,1])
    ):
        r"""Returns the probability of observation conditioned on state and action for each state provided in the state array.

        Args
        ----
        obs: 
            Observation at time t+1.
        state_array: (num_samples, 1)
            Data samples.
        action: 
            Action at time t.
        ...
        Returns
        -------
        p: (num_samples,) Tensor
            Conditional probablities computed using data samples.
        ...
        p = None
        if obs in attention_based_obs:
            if state not in np.arange(no_signal_nodes+1):
                # p.append(0)
                p = 1e-5
                ...
                if obs == 0.5:
                    ...
                    # p.append(0)
                    p = 1e-5
        elif obs == state - 2: #Careful in this part
            # p.append(1)
            p = 1
        else:
            # p.append(0)
