...
class AuditoryForaging(Env):
    ...
    def __init__(self, prob_01 = 0.5, no_signal_nodes = 4, no_penalty_nodes = 1, no_ITI_nodes = 1, lick_cost = -1.0, food_reward = 3.0, high_attention_cost = -0.5, attention_possible = np.array([0, 1]), attention_based_obs=np.array([0,0.5,1]), penalty_cost = -8.0, iti_cost =-4.0):
        """
        Args
        ----
        env_spec:
