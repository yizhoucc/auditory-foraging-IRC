...
print("=== Starting: notebooks/varying p1 smaller.ipynb ===")
...
# --- Cell 1 ---
modelname = 'varyp1_v1'
p1list=[np.array([.5, .75]),np.array([.55, .75]),np.array([.6, .75]),np.array([.65, .75])]

modelname = 'varyp1_v2'
p1list=[np.array([.5, .7]),np.array([.53, .7]),np.array([.56, .7])]
...
for i in range(epochtouse,epochtouse+1):
    for food_reward_idx in [0,1,2,3,4]: # one plot
...
print("=== Done: notebooks/varying p1 smaller.ipynb ===")
