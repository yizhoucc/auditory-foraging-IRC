...
print("=== Starting: notebooks/vary p1 v4.ipynb ===")
...
# --- Cell 3 ---
modelname = 'varyp1v4'
plist = [np.array([.5, .75]), np.array([.55, .75]),
         np.array([.6, .75]), np.array([.65, .75])]

epoch_size = 33333
n_epoch = 22
n_seed = 1
seed = 0
no_episodes = 1555  # for eval plot during training
...
for iepoch in range(0, n_epoch):
    ...
    for _, food_reward_idx in enumerate(use_reward):
        ...
        ticks = [0, 0.5, 1]
        tick_labels = [f'Low p1={plist[0][0]}',
                        '', f'High p1={plist[-1][0]}']
        color_bar.set_ticks(ticks)
        color_bar.set_ticklabels(tick_labels)

        plt.suptitle(
            f'varying p1, p2={p1p2[1]}, food reward={food_reward_list[food_reward_idx]:.0f}')
        plt.tight_layout()
...
for iepoch in range(5,6):
    ...
    for _, food_reward_idx in enumerate(use_reward):
        ...
        for i, p1p2 in enumerate(plist):
            ...
            xs = xs-len(xs)//2
            ax1.plot(xs, autocorr,
                    label=f'food reward={food_reward_list[food_reward_idx]:.0f}', color=cmap(np.linspace(0, 1, len(use_reward)))[i], linewidth=3)
            # trial len dist
            sns.kdeplot([len(a) for a in att_seq_list],
                        fill=False, bw_adjust=0.5, ax=ax2, color=cmap(np.linspace(0, 1, len(use_reward)))[i])
            # print(autocorr)
...
print("=== Done: notebooks/vary p1 v4.ipynb ===")
