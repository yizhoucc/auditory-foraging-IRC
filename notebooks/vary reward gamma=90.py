...
print("=== Starting: notebooks/vary reward gamma=90.ipynb ===")
...
# --- Cell 1 ---
modelname = 'vary_reward_gamma90_V1'
...
for iepoch in range(0, n_epoch):
        ...
        for p1p2 in plist:
            ...
            if iepoch == 0:
                model = PPO('MlpPolicy', taskbelief, verbose=0, device='cpu',
                            clip_range=0.1, ent_coef=0.01, gamma=0.9)
            else:
                previous_model = f'seed{seed}_{modelname}_n{node}_p{p1p2}_ep{iepoch-1}'
                model = PPO.load(f'ycstore/{previous_model}', env=taskbelief,gamma=0.9)
            ...
            model = PPO.load(f'ycstore/{thismodel}', env=taskbelief, gamma=0.9) # test load
    ...
    for inode, node in enumerate(node_list):
        for ip, p1p2 in enumerate(plist):
            ...
            task.obs_certainity_possible = p1p2
            thismodel = f'seed{seed}_{modelname}_n{node}_p{p1p2}_ep{iepoch}'
            model = PPO.load(f'ycstore/{thismodel}',gamma=0.9)
...
print("=== Done: notebooks/vary reward gamma=90.ipynb ===")
