...
print("=== Starting: notebooks/vary reward gamma control.ipynb ===")
...
# --- Cell 1 ---
modelname = 'vary_reward_gamma_ctrl_V1'
...
# --- Cell 2 ---
modelname = 'vary_reward_gamma_ctrl_V2'
...
def hit_reaction_plot(df):
    ...
    for i, (food_reward_idx, curve) in enumerate(data):
        sns.kdeplot(curve, label=f'reward:{food_reward_list[food_reward_idx]:.0f}', clip=(1, np.max(v)), bw_adjust=1,color=rewardcmap(np.linspace(0, 1, len(data)))[i])
...
print("=== Done: notebooks/vary reward gamma control.ipynb ===")
